...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    // 改变购物车中商品的数量
    changeProductAmount(state, action: PayloadAction<{ sku: string; size: string; amount: number }>) {
    ...
    // 增加购物车中商品的数量
    increase(state, action: PayloadAction<{ sku: string; size: string; amount?: number }>) {
    ...
    // 减少购物车中商品的数量
    decrease(state, action: PayloadAction<{ sku: string; size: string; amount?: number }>) {
      const { sku, size, amount = 1 } = action.payload
      if (state.products[sku]?.[size]) {
        const newAmount = Math.max(0, state.products[sku][size].amount - amount)
        if (newAmount > 0) {
...
// 导出购物车的 actions
export const { showCart, addProduct, changeProductAmount, increase, decrease, removeProduct, clearCart } =
  cartSlice.actions
