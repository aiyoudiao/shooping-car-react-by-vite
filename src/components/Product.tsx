import { Badge, Button, Card, ConfigProvider, Segmented, SegmentedProps, Skeleton, message, Image } from 'antd'
...
const ProductImage: FC<ProductImageProps> = ({ sku }) => {
  const image1 = `/shooping-car-react-by-vite/static/products/${sku}-1-product.webp`
  const image2 = `/shooping-car-react-by-vite/static/products/${sku}-2-product.webp`
...
export const LightSegmented = (
  prpos: JSX.IntrinsicAttributes & SegmentedProps & RefAttributes<HTMLLegendElement | HTMLAnchorElement>
) => {
  return (
    <ConfigProvider
      theme={{
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center">
          ...
          <div className="flex justify-end w-full -mr-12">
            <LightSegmented
              ...
              options={data.availableSizes.map(size => {
                return {
                  label: (
                    <div className={`flex items-center justify-center w-5 h-7 rounded-full m-0.5 text-white text-xs`}>
                      {size} 码
                    </div>
                  ),
                  value: size
        ...
        <p className="min-h-14 text-lg font-semibold line-clamp-2" title={data.title}>
          {data.title}
        </p>
        <p className="mb-1">
